...
| Dialogue character art | Local `create-image --anime` pipeline (Anima Base V10 anime workflow) | BiRefNet General preferred; ISNet Anime is an acceptable comparison pass | source art `assets/hero-portrait.png`, `assets/village-cg-{face}.png`; full-body dialogue copies `assets/dialogue/hero.png`, `assets/dialogue/npc-{face}.png` |
...
|---|---|---|
| `assets/hero-walk.png` | `288 × 512`; 3 columns × 4 rows; each frame `96 × 128`; rows are down, left, right, up | Built from the user-supplied `walk_final.gif` step poses in cols 0/2 and `frontwalk_tile2.jpg`'s middle pose in col 1; each source uses BiRefNet General semantic matting followed by a 2px alpha-only erosion to remove halo fringe (never RGB keying). The source is left-facing: movement cycles at 8 fps, every stop uses unmirrored col 1, and only active rightward movement is horizontally mirrored. |
| `assets/npcs/npc-idle-{0..18}.png` | `288 × 128`; three `96 × 128` transparent frames from one identity-consistent generated source | Loaded from each NPC's `face`; runtime uses the documented `NPC_REST` frame and applies only a subtle procedural breathing sway. Displayed at `66 × 94`. |
| `assets/village-cg-{0..18}.png` | Independent transparent full-body PNG, approximately 1080 px tall; never a scaled map sprite | The NPC's numeric `face` selects the file in `interact()`. Copy the complete transparent body into `assets/dialogue/npc-{face}.png`; only empty outer alpha padding may be trimmed — never crop, scale-normalize, or otherwise remove body pixels. `assets/dialogue/hero.png` remains fixed on the left while the matching NPC art stays on the right; inward 9° perspective makes them face one another. Each full-body portrait extends behind the story box, whose z-index deliberately masks the lower half. `#story` owns dialogue text. |
...
- Babylon always renders at the iPhone 14 Pro Max native panel resolution rotated to landscape, `2796 × 1290`, with a fixed 2796:1290 composition. Keep FXAA disabled. Depth-of-field uses Babylon's Low blur tier, focusing the near street at `6500` with `focalLength = 55` and `fStop = 7.5`, so foreground sprites remain readable while distant roofs soften; do not increase it enough to erase pixel silhouettes. CSS letterboxes that landscape frame inside both portrait and landscape browser viewports.
...
The player explores a continuous 20 × 72-unit 3D fantasy town made of three connected districts: South Wind Gate, Hanamori Central Square, and Riverside Shopping Street. Eleven flat pixel-building designs are reused across 26 billboard placements while roads, grass, river, bridge, shadows, depth and collisions exist in 3D. A portrait orthographic camera follows movement on the X/Z plane and keeps both sides of the street visible on iPhone. The protagonist has four-direction, three-frame walking animation. Because Babylon loads the sheet with inverted V coordinates, the authored top-to-bottom facing rows map at runtime as `{down:3,left:2,right:1,up:3}`. The canonical source faces left; upward/downward movement and all idle frames remain unmirrored, while an active rightward walk is horizontally mirrored. The back-facing artwork is retired and must never be displayed. Movement runs at 280 px/s equivalent and uses animation built from consistent master poses rather than cross-generation morphs. Nineteen NPCs each have a three-frame idle sheet, a four-line story, and a transparent full-body dialogue CG; faces 16–18 are the higher-detail main-route cast documented in `DESIGN.md`. The game intentionally has no background music or audio controls.
