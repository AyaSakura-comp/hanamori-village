...
| `assets/village-cg-{0..5}.png` | Independent transparent full-body PNG, approximately 1080 px tall; never a scaled map sprite | The NPC's numeric `face` selects the file in `interact()`. The source is normalized into a transparent 900×900 `assets/dialogue/npc-{face}.png` crop. Every dialogue PNG's non-transparent alpha bounds must be rescaled to exactly 780 px high and bottom-aligned before shipping; canvas dimensions alone are not evidence of equal character size. `assets/dialogue/hero.png` remains fixed on the left while the matching NPC crop stays on the right; inward 9° perspective makes them face one another. The portrait stage reaches behind the story box, which deliberately masks lower bodies while preserving readable upper bodies. `#story` owns dialogue text. |
