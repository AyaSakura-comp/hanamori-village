...
| Dialogue character art | Local `create-image --anime` pipeline (Anima Base V10 anime workflow) | BiRefNet General preferred; ISNet Anime is an acceptable comparison pass | source art `assets/hero-portrait.png`, `assets/village-cg-{face}.png`; standardized UI crops `assets/dialogue/hero.png`, `assets/dialogue/npc-{face}.png` |
...
| `assets/village-cg-{0..5}.png` | Independent transparent full-body PNG, approximately 1080 px tall; never a scaled map sprite | The NPC's numeric `face` selects the file in `interact()`. The source is normalized into a transparent tight-alpha dialogue `assets/dialogue/npc-{face}.png` crop. Every dialogue PNG's non-transparent alpha bounds must be rescaled to exactly 480 px wide and top-aligned before shipping; do not retain a padded square canvas, since `object-fit` would shrink the visible character. `assets/dialogue/hero.png` remains fixed on the left while the matching NPC crop stays on the right; inward 9° perspective makes them face one another. Every NPC crop must reach at least the protagonist crop height so its bottom edge stays behind the story box; the panel deliberately masks lower bodies while preserving readable upper bodies. `#story` owns dialogue text. |
