...
- The authored orthographic camera uses `CAM.view = 3.1`, so the player keeps the close-up vertical size. Its height is `5.7` against a 21-unit setback and `targetY = 2.4`, producing a roughly 9° floor angle. Horizontal bounds alone are multiplied by `WIDE_ANGLE = 1.6`, revealing 60% more town at the sides without shrinking the player.
