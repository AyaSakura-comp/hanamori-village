...
| `assets/village-cg-{0..5}.png` | Independent transparent full-body PNG, approximately 1080 px tall; never a scaled map sprite | The NPC's numeric `face` selects the file in `interact()`. The source is normalized into a transparent 900×900 `assets/dialogue/npc-{face}.png` half-body crop. `assets/dialogue/hero.png` remains fixed on the left while the matching NPC crop stays on the right; equal frames and inward 9° perspective keep both similarly sized and facing one another. `#story` owns dialogue text. |
