...
- Foreground billboards form three staggered rows at world Z `9.4`, `13.4`, and `17.4`, creating a crowded two-to-three-row neighbourhood rather than a single neat lineup. The middle row cycles eighteen dedicated types at roughly four-world-unit intervals; the outer rows reuse the main backdrop facades, which is intentional. Keep row offsets, heights, and flips irregular while preserving a clear central walking lane.
