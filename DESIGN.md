...
- **Dialogue portraits:** show only half-body compositions above the story panel: the protagonist is always anchored on the left and angled slightly right, while the active NPC stays on the right and angles slightly left. Normalize both to equal 900×900 transparent crops with **exactly 780 px visible alpha height** and a shared bottom baseline; do not compensate using unequal CSS scale transforms. The portrait stage extends behind the compact bottom dialogue box (about 98 px minimum height at the reference landscape frame), which intentionally masks lower bodies so the upper half remains the focal point. Source PNGs stay transparent without a white matte.
