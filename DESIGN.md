...
- **Dialogue portraits:** show only half-body compositions above the story panel: the protagonist is always anchored on the left and angled slightly right, while the active NPC stays on the right and angles slightly left. Normalize both to tight-alpha transparent upper-body crops with **exactly 480 px visible alpha width** and a shared top edge; do not compensate using unequal CSS scale transforms or padded square canvases. The portrait stage extends behind the compact bottom dialogue box (about 98 px minimum height at the reference landscape frame), so only head-to-waist/chest artwork is visible above it. Source PNGs stay transparent without a white matte.
