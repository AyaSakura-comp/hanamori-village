...
function loadAssets() {
 ...
 npcs.forEach((n, i) => {
  n.sprite = createBillboard(`npc-${i}`, `assets/npcs/npc-idle-${i}.png?v=route-2`, n.x, n.z, 1.93, 2.67);
...
function createEnvironmentEffects() {
 ...
 ps.emitter = new BABYLON.Vector3(0, 3, 0); ps.minEmitBox = new BABYLON.Vector3(-40, -2, -8); ps.maxEmitBox = new BABYLON.Vector3(40, 5, 8);
 ps.color1 = new BABYLON.Color4(1, .78, .38, .82); ps.color2 = new BABYLON.Color4(1, .93, .7, .48); ps.minSize = .045; ps.maxSize = .17; ps.minLifeTime = 7; ps.maxLifeTime = 14; ps.emitRate = 48; ps.direction1 = new BABYLON.Vector3(-.06, .02, 0); ps.direction2 = new BABYLON.Vector3(.08, .08, 0); ps.minEmitPower = .05; ps.maxEmitPower = .14; ps.blendMode = BABYLON.ParticleSystem.BLENDMODE_ADD; ps.start();
}
...
function interact() { const npc = nearest(); if (!npc) return; activeNpc = npc; line = 0; talking = true; document.querySelector('#hint').classList.remove('show'); document.querySelector('#story').className = 'active'; document.querySelector('#story-cg').src = `assets/dialogue/npc-${npc.face}.png?v=full-2`; document.querySelector('#story-name').textContent = npc.name; advanceStory(); }
