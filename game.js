...
function applyCameraShaders(s){const cam=s.cameras.main;if(!cam.filters)return;cam.filters.external.addVignette(.5,.5,.82,.22);document.querySelector('#app').classList.add('shader-fx');}
...
function create(){
 ...
 player=this.physics.add.sprite(500,3260,'heroWalk',1).setDisplaySize(64,86).setDepth(3260).setCollideWorldBounds(true);player.body.setSize(34,22).setOffset(31,101);this.physics.add.collider(player,walls);if(player.enableFilters){player.enableFilters();player.filters.internal.addShadow(-4,6,.12,1,0x172820,4,.38);}
 npcs.forEach((n,i)=>{n.sprite=this.add.sprite(n.x,n.y,n.texture).setDisplaySize(66,94).setDepth(n.y).play(`idle-${i}`);});
 this.cameras.main.setBounds(0,0,WORLD.w,WORLD.h);this.cameras.main.startFollow(player,true,.13,.13);this.cameras.main.setZoom(.7);this.cameras.main.setRoundPixels(true);applyCameraShaders(this);
